 ...
 * is handled per cut.
 *
 * Everything — videos, posters and PDFs — is served by the site itself
 * from public/projecs, and the videos are ordinary git-tracked files like
 * the rest. They used to live in Vercel Blob because git couldn't get them
 * onto a Vercel build; the site now ships as a Docker image to a VPS, and
 * the store stopped serving them (403) in any case. File names are
 * case-sensitive on the server, so a name here must match the file exactly.
 *
